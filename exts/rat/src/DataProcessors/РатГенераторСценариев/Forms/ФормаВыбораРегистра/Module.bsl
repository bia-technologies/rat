...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Колонка Из Схема.ПакетЗапросов[0].Колонки Цикл
		...
		Если ПустаяСтрока(ИмяКолонки) Тогда
			...
			Если НЕ ПустаяСтрока(ИмяКолонки) И Найти(ИмяКолонки, ".") > 0 Тогда
				ЧастиИмени = СтрРазделить(ИмяКолонки, ".");
				ИмяКолонки = ЧастиИмени[ЧастиИмени.Количество() - 1];
			КонецЕсли;
		КонецЕсли;
		...
		Попытка
			Элемент = Элементы.Добавить("Список" + ИмяКолонки, Тип("ПолеФормы"), Элементы.Список);
			Элемент.ПутьКДанным = "Список." + ИмяКолонки;
			Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Исключение
			РатОбщийКлиентСервер.СообщитьПользователю("Не удалось добавить колонку: " + ИмяКолонки);
		КонецПопытки;
